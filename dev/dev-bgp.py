...
def show(int_dict,int_dict2,int_dict3,int_dict4,int_dict5):
 ...
 print "+--"+'-'*118+"--+"
 print "|{0:16} |{1:12} |{2:10} |{3:10} |{4:10} |{5:55}|".format('BGP Nei','State','Up for','PfxReceived','BestPaths','Description')
 print "+--"+'-'*118+"--+"
...
def parse(lista):
	for line in lista:
		...
		bgpnei = re.search('^BGP neighbor is (.+)', line)
		if bgpnei:
		...
		state = re.search('BGP state = (.+), up for (.+)', line)
                if state and INT:
                        joc=INT[0]
                        int_dict[joc] = state.group(1)
			int_dict2[joc] = state.group(2)		

		prefixes = re.search('^  (.+) accepted prefixes, (.+) are bestpaths', line)
		if prefixes and INT:
			joc=INT[0]
...
if __name__=='__main__':
	...
	python script.py <rMPLSPolo1BR2> <rMPLSVillaSalvadorBR1>
