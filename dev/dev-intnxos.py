...
def show(int_dict,int_dict2,int_dict3):
 print " +--"+"-"*119+"-+"
 print " | %20s | %20s | %20s | %20s | %20s |" % (HOSTNAME,HOSTNAME,HOSTNAME,HOSTNAME,HOSTNAME) 	
 print " +--"+"-"*119+"-+"
 print " |%-20s |%14s| %14s| %-68s|" % ('Interface','Input(Mbps)','Output(Mbps)','Description')
 print " +--"+"-"*119+"-+"
  ...
  if(float(int_dict[k])>0.9 or float(int_dict2[k])>0.9):
   if (float(int_dict[k])>100 or float(int_dict2[k])>100):
    if int_dict[k]!=str(HIGH):
     print " |%-20s*|%14s| %14s| %-68s|" % (k, int_dict[k], int_dict2[k], int_dict3[k])
    else:
     print " |%-19s>*|%14s| %14s| %-68s|" % (k, int_dict[k], int_dict2[k], int_dict3[k])
...
def parse(lista):
	for line in lista:
		...
		interface = re.search(r"^(.+) is up", line)
		if interface:
			INT.append(interface.group(1))
			int_dict3[INT[0]] ='None'
		...
		intrate = re.search(r"30 seconds input rate (.+) bits/sec", line)
		if intrate and INT:
			joc=INT[0]
		...
		outrate = re.search(r"30 seconds output rate (.+) bits/sec", line)
		if outrate and INT:
			joc=INT[0]
	...
	for device in HOSTS:
		try:
			client = pm.SSHClient()
                        ...
                        stdin.write('''
terminal length 0
sh interface | i port-channel|Description|rate|Ethernet
exit
                                ''')
